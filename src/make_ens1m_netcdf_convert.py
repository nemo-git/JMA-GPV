    ...
    if not hits_1w or not hits_2w:
        ...
        for pat in patterns:
            for p in base_dir.rglob(pat):
                s = str(p)
                if "ens_1w" in s or "1wEGPV" in s:
                    hits_1w.append(p)
                elif "ens_2w1m" in s or "2wEGPV" in s:
                    hits_2w.append(p)
...
def _find_monthly_paths_for_date(base_dir: Path, target_yyyymmdd: str, is_upper: bool, debug=False):
    """
    ens_2w1m から target_yyyymmdd の 00UTC 初期 monthly(EPSC) Lsurf/L-pall を収集。
    FD の分割本数は配信によって異なる可能性があるため、該当日付の全パートを拾って返す。
    """
    yyyy = target_yyyymmdd[:4]
    if is_upper:
        patterns = [
            f"Z__C_*_{target_yyyymmdd}000000_EPSC_*_L-pall_*_grib2.bin",
            f"*{target_yyyymmdd}000000*EPSC*L-pall*grib2.bin",
            f"Z__C_*_{target_yyyymmdd}000000_EPSC_*_Lpall_*_grib2.bin",
            f"*{target_yyyymmdd}000000*EPSC*Lpall*grib2.bin",
        ]
    else:
        patterns = [
    ...
    if debug:
        ...
        print(f"[SCAN monthly] date={target_yyyymmdd} init=00UTC layer={layer}")
        for h in hits: print("   ", h)

    if not hits:
        raise FileNotFoundError(f"[monthly] {target_yyyymmdd} のファイルが見つかりません")
    return hits
...
def concat_time_and_save(paths, element: str, out_path: Path, debug=False, level_hpa: int|None=None):
    ...
    origin = np.datetime64("1900-01-01T00:00:00")
...
def main(argv=None):
    ...
    ap = argparse.ArgumentParser(description="ENS1M Lsurf/L-pall/monthly → NetCDF（pygrib, 51/25ens, 1日前/週次/月次）")
    ap.add_argument("--date", required=True, help="基準日 yyyymmdd（1w2w: この1日前12UTC, monthly: 木曜に火/水12UTCを処理）")
    ap.add_argument(
        "--dir",
        default=DEFAULT_DIR,
    ...
    if do_monthly:
                ...
                for p in paths_1m:
                    try:
                        ...
                        if "対象メッセージが見つかりません" in str(e):
                            print(f"[WARN] monthly skip (no {element}@{lvl}) in {p.name}")
                        else:
                            raise
            else:
                for p in paths_1m:
                    try:
                        ...
                        if "対象メッセージが見つかりません" in str(e):
                            print(f"[WARN] monthly skip (no {element}) in {p.name}")
                        else:
                            raise

            if not das:
                print(f"[WARN] monthly {tgt}: {element}{'' if not is_upper else f'@{lvl}'} が1件も見つかりません。スキップします。")
                continue

            print(f"[INFO] monthly 読み込み検証 OK ({element}{'' if not is_upper else f'@'+str(lvl)}) for {tgt}  (used {len(das)}/{len(paths_1m)} parts)")
        ...
        _dbg(args.debug, "[INFO] monthly はスキップ（木曜でない、または --skip-monthly 指定）")
