#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Wait for GEPS raw delivery files, then run the daily batch.
...
- on Thursdays, monthly 1m Lsurf files must also exist for Tuesday and Wednesday 12UTC
"""
...
def _monthly_hits(base_dir: Path, target_yyyymmdd: str) -> list[Path]:
    ...
    if not hits:
        fallback: list[Path] = []
        for pat in patterns:
            fallback.extend(sorted(base_dir.rglob(pat)))
...
def _build_readiness_report(run_date_yyyymmdd: str, base_dir: Path) -> tuple[bool, list[str]]:
    ...
    if proc_date.weekday() == 3:
        ...
        lines.extend(
            [
                f"monthly 1m: {len(hits_tue)} file(s) for {tue}12UTC Lsurf",
                f"monthly 1m: {len(hits_wed)} file(s) for {wed}12UTC Lsurf",
            ]
        )
...
def _run_batch(args: argparse.Namespace, target_date: str) -> int:
    ...
    cmd = [
        sys.executable,
        str(scripts_dir / "00_run_geps_batch.py"),
        "--date",
        target_date,
        "--vars",
        args.vars,
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Wait for GEPS delivery files, then run the batch.")
    ...
    ap.add_argument("--vars", default=DEFAULT_VARS, help=f"Variables to pass to 00_run_geps_batch.py (default: {DEFAULT_VARS})")
